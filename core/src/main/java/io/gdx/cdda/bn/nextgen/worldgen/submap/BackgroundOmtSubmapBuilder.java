...
public final class BackgroundOmtSubmapBuilder {
    ...
    private static final String UNDERBRUSH = "t_underbrush";
    private static final String TREE = "t_tree";
    private static final String YOUNG_TREE = "t_tree_young";
    ...
    public static Optional<MapGrid> buildIfSupported(
        ...
        if (isRoadOmt(normalized, connectionRegistry)) {
            return Optional.of(buildRoad(overmap, omtX, omtY, connectionRegistry, oterRegistry, rng));
        }
        if (isWaterOmt(normalized)) {
            return Optional.of(buildWater(normalized, rng));
        }
        if (isForestOmt(normalized)) {
            return Optional.of(buildForest(rng));
        }
        if (isFieldOmt(normalized)) {
            return Optional.of(buildField(rng));
        }
        if (isBeachOmt(normalized)) {
            return Optional.of(buildBeach(rng));
        }
        if (isOpenLandOmt(normalized)) {
            return Optional.of(new MapGrid(SIZE, SIZE, GRASS));
        }
        return Optional.empty();
    }

    private static MapGrid buildField(final Random rng) {
        final MapGrid grid = new MapGrid(SIZE, SIZE, GRASS);
        scatterVegetation(grid, rng, 14, UNDERBRUSH, 6);
        return grid;
    }

    private static MapGrid buildForest(final Random rng) {
        final MapGrid grid = new MapGrid(SIZE, SIZE, GRASS);
        for (int y = 0; y < SIZE; y++) {
            for (int x = 0; x < SIZE; x++) {
                ...
                if (roll > 13) {
                    ...
                    grid.setTerrain(x, y, YOUNG_TREE);
                } else if (roll > 9) {
                    grid.setTerrain(x, y, UNDERBRUSH);
                }
            }
        }
        return grid;
    ...
    private static boolean isRoadOmt(final String normalized, final OvermapConnectionRegistry connectionRegistry) {
        if (connectionRegistry != null
            && OvermapConnectionResolver.connectionIdForOmt(normalized, connectionRegistry).isPresent()) {
            return true;
        }
